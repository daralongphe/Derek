...
public:
	...
	//Function to determine the number of nodes in a binary tree that have only one child.
	int treeSingleParentCount() const
	{
...
private:
	...
	int leavesCount(nodeType<elemType>* p) const
	{
		...
		{
				return ((p->lLink == nullptr && p->rLink == nullptr) ? 1 : 0) + leavesCount(p->lLink) + leavesCount(p->rLink);
		}
	}
	...
	int singleParent(nodeType<elemType>* p) const
	{
		...
		{
			return (((p->lLink == nullptr && p->rLink != nullptr) || (p->lLink != nullptr && p->rLink == nullptr)) ? 1 : 0) + singleParent(p->lLink) + singleParent(p->rLink);
		}
	}
